# AGENTS.md — tests/capture/

- Mirrors `src/democreate/capture/`; a new module there needs a `test_*.py` here.
...
- Run scoped: `.venv/bin/python -m pytest -q tests/capture/` (no `--cov`).
